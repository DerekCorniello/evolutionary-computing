...
let genome_init (size : int) (rng_state : Rand.rng_state) (fitness_fn : genome_t -> float) : genome_t =
...
let population_init (member_count : int) (member_length : int) (rng_state : Rand.rng_state) (fitness_fn : genome_t -> float) : population_t =
  (* create array of genomes *)
  { size = member_count; members = Array.init member_count (fun _ -> genome_init member_length rng_state fitness_fn) }
...
let genome_mutate (genome : genome_t) (mutation_rate : float) (rng_state : Rand.rng_state) (fitness_fn : genome_t -> float) : unit =
...
let population_mutate (population : population_t) (mutation_rate : float) (rng_state : Rand.rng_state) (fitness_fn : genome_t -> float) : unit =
  Array.iter (fun member -> genome_mutate member mutation_rate rng_state fitness_fn) population.members

let genome_1P_crossover (genome1 : genome_t) (genome2 : genome_t) (rng_state : Rand.rng_state) (fitness_fn : genome_t -> float) : (genome_t * genome_t) =
  if genome1.length <> genome2.length then failwith "genome1 and genome2 are not the same size";
  ...
  ({ length = genome1.length; fitness = 0.0; string = child1 } |> fun g -> { g with fitness = fitness_fn g }),
  ({ length = genome2.length; fitness = 0.0; string = child2 } |> fun g -> { g with fitness = fitness_fn g })

(* ------------------ generation creation ------------------ *)

let make_new_generation_with (curr_pop : population_t) 
                             (mutation_rate : float) 
  ...
  let min_fitness, sum_fitness =
    Array.fold_left (fun (min_acc, sum_acc) score -> (min min_acc score, sum_acc +. score))
  ...
  let new_pop = { size = curr_pop.size; members = Array.make curr_pop.size { length=0; fitness=0.0; string=[||] } } in
  for i = 0 to curr_pop.size - 2 do
    ...
    let child1, child2 =
      if Rand.uniform rng_state < crossover_rate then genome_1P_crossover child1 child2 rng_state fitness_fn
      else child1, child2
    in
