...
let decode_genome (genome : SGA.genome_t) =
    ...
    let decode_slice start_idx =
        ...
        (* scale to range [-2.048, 2.048] *)
        let min_val = -2.048 in
        let max_val_range = 2.048 in
        min_val +. ((float_of_int !value) /. max_val) *. (max_val_range -. min_val)
    in
...
let make_new_generation_with_uniform (pop : SGA.population_t) (mutation_rate : float)
    (crossover_rate : float) (rng_state : Rand.rng_state)
    (fitness_fn : SGA.genome_t -> float) : SGA.population_t =
  ...
  new_members.(0) <- { (SGA.copy_genome members.(0)) with SGA.fitness = members.(0).SGA.fitness };
  ...
  for i = 1 to pop_size - 1 do
    ...
    let tournament = 
      Array.init tournament_size (fun _ -> 
        let idx = int_of_float (Rand.uniform_range rng_state 0.0 (float_of_int pop_size -. 0.1)) in
        members.(idx)
      )
    in
    (* for minimization, we want the individual with the lowest fitness *)
    let parent1 = Array.fold_left (fun best g -> 
      if g.SGA.fitness < best.SGA.fitness then g else best) tournament.(0) tournament in
    ...
    in
    let parent2 = Array.fold_left (fun best g -> 
      if g.SGA.fitness < best.SGA.fitness then g else best) tournament.(0) tournament in
    ...
    let child1, child2 = 
      if Rand.uniform rng_state < crossover_rate then
        ...
        (SGA.copy_genome parent1, SGA.copy_genome parent2)
    in
...
let () =
    ...
    let high_fitness_ratio = 0.98 in
        ...
        let final_population, termination_reason, generations =
            let rec evolve current_pop generation last_avg_fitness
                ...
                if generation >= max_generations then
                  (current_pop, "reached maximum generation limit", max_generations)
                else
